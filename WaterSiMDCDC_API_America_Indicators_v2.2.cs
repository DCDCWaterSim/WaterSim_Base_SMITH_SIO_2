...
namespace WaterSimDCDC
{
    ...
    public class IndicatorDataClass
    {
        ...
        public int TotalPortfolio(int State)
        {
            int result = -1;
            bool iserr = true;
            string errMessage = "";
...
//            double EperP = EFfluent / Pop;
    ...
    // February 2017 das
    // -------------------------------
    #region Groundwater Pumping Indicator


    ///-------------------------------------------------------------------------------------------------
    /// <summary>   A gw pumping indicator process. </summary>
    ///
    /// <remarks>   DAS:  2/10/2017. </remarks>
    ///-------------------------------------------------------------------------------------------------

    public class GWPumpingIndicatorProcess : AnnualFeedbackProcess
    {
        int FStateCode = 0;
        int FProportion = 0;
        double FGoal = 0;
        int FTotal = 0;
        double FTotalPortfolio = 0;
        int FBaseWithdrawal = 0;
        int FAnnualRatio = 0;
        int FCumulativeRatio = 0;
        ...
        public GWPumpingIndicatorProcess(string aName, WaterSimManager WSim, IndicatorDataClass iData)
        {
            BuildDescStrings();
            Fname = aName;
            this.Name = this.GetType().Name;
        ...
        {
            FProcessDescription = "";
            FProcessLongDescription = "The Amount of groundwater pumped relative to the total portfolio";
            FProcessCode = "CSY";
        }
        ...
        public override bool ProcessStarted(int year, WaterSimManagerClass WSim)
        {
            ...
            // get the gola for this state
            //FGoal = FIData.SafeYieldGoal(statecode);
            //FGoal = FIData.PumpingGoal(statecode);
            //FTotal = FIData.TotalPortfolio(statecode);
            // get the initial withdrawal
            FBaseWithdrawal = WSim.ParamManager.Model_Parameter(eModelParam.epGroundwater).Value;
            FTotal = WSim.ParamManager.Model_Parameter(eModelParam.epTotalSupplies).Value;
            double DTotal = FTotal;
            // calulate SafeYield Withdrawal
            if(0 < DTotal)
            FTotalPortfolio = FBaseWithdrawal / DTotal;
            try
            {
               // FMaxWithdrawAsInt = Convert.ToInt32(FMaxWithdraw);
            }
        ...
        {
            // get the current total GW withdrawal
            double GWWithdrawal = WSim.ParamManager.Model_Parameter(eModelParam.epGroundwater).Value;
            double TotalUsed = WSim.ParamManager.Model_Parameter(eModelParam.epTotalSupplies).Value;
            // Calculate cumulative net SY defecit
           // int AnnualNet = GWWithdrawal / TotalUsed;
            double AnnualProportion = 0;

            if (0 < TotalUsed)AnnualProportion= GWWithdrawal / TotalUsed; 
            //
            FProportion += Convert.ToInt32(AnnualProportion);
            // setup for rations
            double GW = GWWithdrawal;
            double Goal = FGoal;
            double CumulativeAnnual = FProportion;
            // calcualte ratios
            double AnnualRatio = 0;
            double CumulativeRatio = 0;
            //if (GW != 0)
            //{
                AnnualRatio = AnnualProportion;
                CumulativeRatio = CumulativeAnnual ;
            //}
            // do double to int conversion as percent
            int tempAnnualRatio = 0;
            int tempCumulativeRatio = 0;
            try
            {
                tempAnnualRatio = Convert.ToInt32(AnnualRatio * 100);
            }
            catch
            {
                tempAnnualRatio = 100;
            ...
            try
            {
                tempCumulativeRatio = Convert.ToInt32(CumulativeRatio * 100);
            }
            catch
            {
                tempCumulativeRatio = 100;
        ...
        public int Goal
        {
            get { return Convert.ToInt32(FGoal); }
        }
        ...
        public int CumulativeNet
        {
            get { return FProportion; }
        }
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Gets the state code. </summary>
        ///
    ...
    public class WebIndicator_GroundWaterPumping : WebIndicator
    {
       // const int eGWAnnualPumping_Indicator = 1;
 
        //      int FBaseParam = 0;
        GWPumpingIndicatorProcess FGWProcess = null;
        ...
        public WebIndicator_GroundWaterPumping(WaterSimManager WSim, IndicatorDataClass iData)
        {

            FWsim = WSim;
            FGWProcess = new GWPumpingIndicatorProcess("Groundwater Pumping Indicator", FWsim, iData);
            FWsim.ProcessManager.AddProcess(FGWProcess);
            CreateModelParameters();
        }
        ...
        public int geti_GWPumpingIndicator()
        {
            int value = 0;
            if (FGWProcess != null)
            {
        ...
        public int StateCode()
        {
            ...
            if (FGWProcess != null)
            {
                value = FGWProcess.CumulativeNet;
            }
            return value;
        ...
        public void CreateModelParameters()
        {
            ...
            //FPM.AddParameter(new ModelParameterClass(eModelParam.eGWAnnualPumping_Indicator, "Groundwater Pumping Indicatior", "GWPUMP", geti_GWPumpingIndicator));
            //ExtendDoc.Add(new WaterSimDescripItem(eModelParam.eGWAnnualPumping_Indicator, "The ratio of groundwater pumped to total water portfolio", "%", "Percent", "Groundwater Pumping Indicator", new string[] { }, new int[] { }, new ModelParameterGroupClass[] { }));
    ...
    public static partial class eModelParam
    {
        ...
        // testing WSA West indicators
        public const int eGWAnnualPumping_Indicator = 206;
    ...
    public partial class WaterSimManager : WaterSimManagerClass
    {
        ...
        // ---------------------------------------------
        // Testing WSA West indicators
        protected WebIndicator_GroundWaterPumping GWPumpingIndicator;
        //
        //---------------------------------------------------
        // Total Demand
        // ------------------------------------------------
 ...
 //           int TempInt = Convert.ToInt32(Math.Round (WSmith.TheCRFNetwork.Urban.Demand + WSmith.TheCRFNetwork.Agriculture.Demand + WSmith.TheCRFNetwork.Rural.Demand + WSmith.TheCRFNetwork.Industrial.Demand + WSmith.TheCRFNetwork.Power.Demand));
        ...
        void seti_agTargetEfficiency(int value)
        {
            ...
            WaterSimAmericaModel.AgricultureTargetGPDD = value;
        }
...
//                ParamManager.AddParameter(new ModelParameterClass(eModelParam.epAgricultureProduction, "Agriculture Net $", "ANP", geti_AgProduction));
                ...
                //
                // ---------------------------------------------------------------
                // New Indicator testing for WaterSimWest
                GWPumpingIndicator = new WebIndicator_GroundWaterPumping(this, IndicatorData);
                ParamManager.AddParameter(new ModelParameterClass(eModelParam.eGWAnnualPumping_Indicator, "Annual Safe Yield Indicatior", "GWPUMPI", GWPumpingIndicator.geti_AnnualIndicator));
                ExtendDoc.Add(new WaterSimDescripItem(eModelParam.eGWAnnualPumping_Indicator, "The ratio of the amount of reduction in 2010 groundwater withdrawal needed to achieve Safe Yield to the annual ground water withdrawal, 0 means safe yield achived, 100 is max value", "%", "Percent", "Groundwater Indicator", new string[] { }, new int[] { }, new ModelParameterGroupClass[] { }));
